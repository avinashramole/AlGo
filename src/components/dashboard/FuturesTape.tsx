...
export function FuturesTape() {
  ...
  const trade = async (row: (typeof rows)[number], side: "BUY" | "SELL") => {
    ...
    try {
      ...
      setNote(
        result.live
          ? `Sent to Dhan · ${side} ${row.name} · ID ${row.securityId}`
          : result.warning || `Desk fill · ${side} ${row.name} · ID ${row.securityId}`,
      );
    } catch (err) {
  ...
  if (!rows.length) {
    return (
      <section className="card p-4">
        ...
        <p className="mt-2 text-xs text-slate-400">Loading Dhan future security IDs from the scrip master…</p>
      </section>
    );
  }
  ...
  return (
    <section className="card overflow-x-auto p-4">
      <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
        <div>
          <div className="text-sm font-bold">Index futures · live IDs</div>
          <p className="text-xs text-slate-400">
            Front-month and later FUTIDX contracts. BUY/SELL is 1 lot MIS on Dhan when LIVE.
          </p>
        </div>
      ...
      <table className="w-full min-w-[720px] text-left text-xs">
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={`${row.root}-${row.expiry}-${row.securityId}`} className="soft-row">
